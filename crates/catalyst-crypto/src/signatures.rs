...
impl Signature {
    ...
    pub fn from_bytes(bytes: [u8; SIGNATURE_SIZE]) -> CryptoResult<Self> {
        ...
        let s = Scalar::from_canonical_bytes(s_bytes).into_option().ok_or(
            CryptoError::InvalidSignature("Invalid S scalar".to_string()),
        )?;
...
impl SignatureScheme {
    ...
    pub fn sign<R: RngCore + CryptoRng>(
        ...
        let challenge_data = [&r.compress().0[..], &public_key.to_bytes()[..], message].concat();
