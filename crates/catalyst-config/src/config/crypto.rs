    ...
    pub fn validate(&self) -> ConfigResult<()> {
        ...
        if self.security.constant_time_operations {
            ...
            if !self.security.secure_memory_cleanup {
                return Err(ConfigError::ValidationFailed(
                    "Secure memory cleanup should be enabled when using constant time operations".to_string()
                ));
            }
        }
        ...
        if let Some(refresh_interval) = self.security.key_refresh_interval_ms {
            if refresh_interval < 60000 { // Less than 1 minute
                return Err(ConfigError::ValidationFailed(
        ...
        if self.key_derivation.iterations < min_iterations {
            return Err(ConfigError::ValidationFailed(
                format!("KDF iterations too low for {:?}, minimum: {}", self.key_derivation.kdf, min_iterations)
            ));
        }
    ...
    pub fn get_crypto_strength(&self) -> CryptoStrength {
        ...
        let kdf_score = match self.key_derivation.kdf {
            KeyDerivationFunction::Argon2 => {
                if self.key_derivation.iterations >= 10 { 15 }
                else if self.key_derivation.iterations >= 5 { 10 }
                else { 5 }
            },
            KeyDerivationFunction::Scrypt => {
                if self.key_derivation.iterations >= 32768 { 12 }
                else if self.key_derivation.iterations >= 16384 { 8 }
                else { 4 }
            },
            KeyDerivationFunction::Pbkdf2 => {
                if self.key_derivation.iterations >= 100000 { 10 }
                else if self.key_derivation.iterations >= 10000 { 6 }
                else { 2 }
            },
        };
        score += kdf_score;
        ...
        if self.security.constant_time_operations { score += 5; } else { issues.push("Constant-time operations disabled".to_string()); }
        if self.security.secure_memory_cleanup { score += 5; } else { issues.push("Secure memory cleanup disabled".to_string()); }
        if self.security.side_channel_resistance { score += 5; } else { issues.push("Side-channel resistance disabled".to_string()); }
    ...
    pub fn estimate_performance_impact(&self) -> CryptoPerformanceEstimate {
        ...
        match self.signature_scheme {
            SignatureScheme::MuSig => {
                signing_overhead *= if self.signature_aggregation.enabled { 1.5 } else { 1.2 };
                verification_overhead *= if self.signature_aggregation.batch_aggregation { 0.8 } else { 1.1 };
            },
        ...
        CryptoPerformanceEstimate {
            ...
            memory_overhead_mb: if self.security.secure_memory_cleanup { 2.0 } else { 1.0 },
        }
    }
}
...
impl std::fmt::Display for CryptoStrength {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Crypto Strength: {:?} ({}/{})", self.level, self.score, self.max_score)?;
