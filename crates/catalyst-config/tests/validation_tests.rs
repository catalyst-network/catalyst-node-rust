...
use catalyst_config::networks::{NetworkConfig, NetworkSettings, NodeConfig, ConsensusConfig, StorageConfig, RpcConfig, ServiceBusConfig};
...
fn test_node_config_validation() {
    ...
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::EmptyNodeId)));
    }
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::InvalidLogLevel(_))));
    }
    ...
    for level in valid_levels {
        ...
        assert!(validator.validate(&config).is_ok(), "Log level '{}' should be valid", level);
    }
}
...
fn test_network_config_validation() {
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::EmptyNetworkName)));
    }
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::InvalidPort(_))));
    }
    ...
    for port in [1, 1024, 8080, 9944, 65535] {
        ...
        assert!(validator.validate(&config).is_ok(), "Port {} should be valid", port);
    }
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::InvalidMaxPeers(_))));
    }
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::EmptyBindAddress)));
    }
    ...
    for addr in valid_addresses {
        ...
        assert!(validator.validate(&config).is_ok(), "Address '{}' should be valid", addr);
    }
}
...
fn test_consensus_config_validation() {
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::InvalidCycleTime(_))));
    }
    ...
    for cycle_time in [5000, 15000, 30000, 60000, 120000] {
        ...
        assert!(validator.validate(&config).is_ok(), "Cycle time {} should be valid", cycle_time);
    }
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::InvalidMaxProducers(_))));
    }
    ...
    for max_producers in [10, 50, 100, 500, 1000] {
        ...
        assert!(validator.validate(&config).is_ok(), "Max producers {} should be valid", max_producers);
    }
}
...
fn test_storage_config_validation() {
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::InvalidCacheSize(_))));
    }
    ...
    for cache_size in [32, 64, 128, 256, 512, 1024, 2048] {
        ...
        assert!(validator.validate(&config).is_ok(), "Cache size {} should be valid", cache_size);
    }
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::EmptyStoragePath)));
    }
...
fn test_port_conflict_validation() {
    ...
    assert!(result.is_err());
    if let Err(errors) = result {
        assert!(errors.iter().any(|e| matches!(e, ValidationError::PortConflict(_, _))));
    }
...
fn test_network_id_validation() {
    ...
    for id in valid_ids {
        ...
        assert!(validator.validate(&config).is_ok(), "Network ID {} should be valid", id);
    }
...
fn test_log_level_validation_comprehensive() {
    ...
    for level in case_variants {
        config.node.log_level = level.to_string();
        let result = validator.validate(&config);
        assert!(result.is_err(), "Log level '{}' should be invalid (case sensitive)", level);
    }
...
fn test_data_directory_validation() {
    ...
    for path in valid_paths {
        ...
        assert!(validator.validate(&config).is_ok(), "Path '{}' should be valid", path);
    }
}
...
fn test_validation_performance() {
    ...
    assert!(result.is_ok());
    assert!(duration.as_millis() < 100, "Validation should be fast even with large configs");
}
