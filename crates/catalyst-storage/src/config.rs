    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (name, config) in &self.column_families {
            if config.write_buffer_size < 1024 * 1024 {
                return Err(format!(
                    "Column family {} write_buffer_size must be at least 1MB",
                    name
                ));
            }
            ...
            if config.max_write_buffer_number < 1 {
                return Err(format!(
                    "Column family {} max_write_buffer_number must be at least 1",
                    name
                ));
            }
            ...
            if config.target_file_size_base < 1024 * 1024 {
                return Err(format!(
                    "Column family {} target_file_size_base must be at least 1MB",
                    name
                ));
            }
            ...
            if config.bloom_filter_bits_per_key < 1 || config.bloom_filter_bits_per_key > 30 {
                return Err(format!(
                    "Column family {} bloom_filter_bits_per_key must be between 1 and 30",
                    name
                ));
            }
            ...
            if config.block_size < 1024 {
                return Err(format!(
                    "Column family {} block_size must be at least 1KB",
                    name
                ));
            }
        }
        ...
        if self.transaction_config.transaction_timeout == 0 {
            return Err(
                "transaction_config.transaction_timeout must be greater than 0".to_string(),
            );
        }

        if self.transaction_config.max_concurrent_transactions == 0 {
            return Err(
                "transaction_config.max_concurrent_transactions must be at least 1".to_string(),
            );
        }
...
mod tests {
    ...
    fn test_column_family_configs() {
        ...
        assert!(matches!(
            contract_config.compression_type,
    ...
    fn test_column_family_management() {
        ...
        assert_eq!(
            config
                .get_column_family_config("custom")
                .unwrap()
                .write_buffer_size,
    ...
    fn test_custom_column_family_configs() {
        ...
        let custom = ColumnFamilyConfig::custom(128 * 1024 * 1024, CompressionType::Snappy, 12);
    ...
    fn test_serde_serialization() {
        ...
        assert_eq!(
            config.column_families.len(),
            deserialized.column_families.len()
        );
    }
