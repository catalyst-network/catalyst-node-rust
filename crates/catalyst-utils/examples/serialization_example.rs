...
fn main() -> CatalystResult<()> {
    ...
    impl CatalystSerializeWithContext for ProtocolMessage {
        ...
        {
            if data.len() < 13 {
                // min size: 4 + 1 + 4 + 1 + 4 = 14 bytes, but we check for 13 first
    ...
    println!(
        "V1 recovery successful: {}",
        message.version == recovered_v1.version
    );
    println!(
        "V2 recovery successful: {}",
        message.version == recovered_v2.version
    );
    ...
    println!(
        "Transaction serialization successful: {}",
