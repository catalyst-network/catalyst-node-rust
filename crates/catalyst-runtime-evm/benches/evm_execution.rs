...
fn benchmark_simple_transfer(c: &mut Criterion) {
    ...
    c.bench_function("simple_transfer", |b| {
        b.iter(|| {
            ...
            rt.block_on(async { black_box(runtime.execute_transaction(&tx, &block_env).await) })
        })
    });
}
    ...
    group.bench_function("fee_calculation", |b| {
        ...
        b.iter(|| {
            for gas_used in [21000, 50000, 100000, 500000] {
                black_box(calculator.calculate_fee(gas_used, Some(U256::from(1_000_000_000u64))));
            }
        })
    });
