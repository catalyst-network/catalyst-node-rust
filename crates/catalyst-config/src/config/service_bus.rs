...
impl ServiceBusConfig {
    ...
    pub fn validate(&self) -> ConfigResult<()> {
        ...
        if self.rest_api.port == 0 {
            return Err(ConfigError::ValidationFailed(
                "REST API port cannot be 0".to_string(),
            ));
        }

        if self.websocket.port == self.rest_api.port {
            return Err(ConfigError::ValidationFailed(
                "WebSocket and REST API ports cannot be the same".to_string(),
            ));
        }
        ...
        if self.websocket.max_connections == 0 {
            return Err(ConfigError::ValidationFailed(
                "Max WebSocket connections must be greater than 0".to_string(),
            ));
        }
        ...
        if self.event_processing.buffer.max_batch_size > self.event_processing.buffer.size {
            return Err(ConfigError::ValidationFailed(
                "Max batch size cannot exceed buffer size".to_string(),
            ));
        }
        ...
        if let Some(ref tls) = self.websocket.tls {
            if tls.enabled {
                ...
                if tls.key_file.is_empty() {
                    return Err(ConfigError::ValidationFailed(
                        "TLS key file path cannot be empty".to_string(),
                    ));
                }
            }
        }
        ...
        if self.auth.enabled {
            ...
            if self.auth.jwt.secret_key.is_empty() {
                return Err(ConfigError::ValidationFailed(
                    "JWT secret key cannot be empty".to_string(),
                ));
            }

            if self.auth.jwt.expiration_seconds == 0 {
                return Err(ConfigError::ValidationFailed(
                    "JWT expiration time must be greater than 0".to_string(),
                ));
            }
        }
        ...
        if self.rate_limiting.enabled {
            ...
            if self.rate_limiting.per_client_limits.requests_per_minute == 0 {
                return Err(ConfigError::ValidationFailed(
                    "Per-client rate limit must be greater than 0".to_string(),
                ));
            }
        }
