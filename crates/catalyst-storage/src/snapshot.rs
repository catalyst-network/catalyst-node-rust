...
impl SnapshotManager {
    ...
    pub async fn create_snapshot(&self, name: &str) -> StorageResult<Snapshot> {
        ...
        // Write metadata file
        let metadata_content = serde_json::to_string_pretty(&snapshot).map_err(|e| {
    ...
    async fn load_snapshot_data(&self, snapshot: &Snapshot) -> StorageResult<()> {
        ...
        if !data_dir.exists() {
            return Err(StorageError::snapshot(format!(
                "Snapshot data directory not found for '{}'",
                snapshot.name
            )));
        }
    ...
    pub async fn import_snapshot(
        ...
        let final_name = new_name
            .unwrap_or_else(|| format!("{}_imported_{}", snapshot.name, current_timestamp()));
...
impl Snapshot {
    ...
    pub async fn import_from_directory(name: &str, import_dir: &Path) -> StorageResult<Snapshot> {
        ...
        if !metadata_file.exists() {
            return Err(StorageError::snapshot(format!(
                "Snapshot metadata file not found: {:?}",
                metadata_file
            )));
        }
...
mod tests {
    ...
    async fn test_snapshot_with_data() {
        ...
        assert_ne!(snapshot.state_root(), Hash::from([0u8; 32]));
    ...
    async fn test_snapshot_cleanup() {
        ...
        for i in 0..8 {
            manager
                .create_snapshot(&format!("cleanup_test_{}", i))
                .await
                .unwrap();
