...
fn digital_signatures() -> CryptoResult<()> {
    ...
    println!(
        "✓ Serialized signature verification: {}",
...
fn musig_signatures() -> CryptoResult<()> {
    ...
    println!(
        "  Simulating consensus with {} validators",
        validators.len()
    );
    ...
    println!(
        "✓ Aggregated {} signatures into one",
        musig_signature.participant_count
    );
    ...
    println!(
        ...
        if consensus_valid { "VALID" } else { "INVALID" }
    );
    ...
    println!(
        "  Space savings: {} bytes → {} bytes ({:.1}% reduction)",
        individual_size,
        aggregated_size,
...
fn hash_operations() -> CryptoResult<()> {
    ...
    let combined_hash = blake2b_hash_multiple(&pieces.iter().map(|p| &p[..]).collect::<Vec<_>>());
...
fn confidential_transactions() -> CryptoResult<()> {
    ...
    println!(
        "  Original commitment: {}",
        hex::encode(alice_old_commit.to_bytes())
    );
    println!(
        "  Reconstructed commitment: {}",
        hex::encode(reconstructed.to_bytes())
    );
