use catalyst_config::{CatalystConfig, ConfigLoader, ConfigUtils, NetworkType};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Devnet - P2P Port: {}, Consensus Cycle: {}ms",
        devnet_config.network.p2p_port, devnet_config.consensus.cycle_duration_ms
    );

    let testnet_config = CatalystConfig::new_for_network(NetworkType::Testnet);
    println!(
        "   Testnet - P2P Port: {}, Consensus Cycle: {}ms",
        testnet_config.network.p2p_port, testnet_config.consensus.cycle_duration_ms
    );

    let mainnet_config = CatalystConfig::new_for_network(NetworkType::Mainnet);
    println!(
        "   Mainnet - P2P Port: {}, Consensus Cycle: {}ms\n",
        mainnet_config.network.p2p_port, mainnet_config.consensus.cycle_duration_ms
    );
    ...
    println!(
        "   Loaded configuration - Network: {}",
        loaded_config.network.name
    );
    ...
    println!(
        "   Producer count from env: {}",
        env_config.consensus.producer_count
    );
    ...
    let report =
        catalyst_config::loader::validation::ConfigValidator::generate_report(&devnet_config);
    println!("{}", report);
