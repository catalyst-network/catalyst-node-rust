...
impl TransactionBatch {
    ...
    /// Delete an account
    pub async fn delete_account(&self, address: &Address) -> StorageResult<()> {
        self.check_not_committed()?;

        let operation = BatchOperation::DeleteAccount { address: *address };
    ...
    pub async fn delete_transaction(&self, hash: &Hash) -> StorageResult<()> {
        ...
        let operation = BatchOperation::DeleteTransaction { hash: *hash };
    ...
    pub async fn delete_consensus(&self, key: &[u8]) -> StorageResult<()> {
        ...
        let operation = BatchOperation::DeleteConsensus { key: key.to_vec() };
    ...
    pub async fn delete_contract(&self, address: &Address) -> StorageResult<()> {
        self.check_not_committed()?;

        let operation = BatchOperation::DeleteContract { address: *address };
    ...
    pub async fn delete_dfs_ref(&self, hash: &Hash) -> StorageResult<()> {
        ...
        let operation = BatchOperation::DeleteDfsRef { hash: *hash };
    ...
    fn estimate_operation_size(&self, operation: &BatchOperation) -> usize {
        match operation {
            BatchOperation::PutAccount { .. } => {
                20 + 100 // address(20) + estimated account data size
            }
            BatchOperation::DeleteAccount { .. } => 20,
            BatchOperation::PutTransaction { data, .. } => 32 + data.len(), // hash + data
            BatchOperation::DeleteTransaction { .. } => 32,
            BatchOperation::PutMetadata { key, value } => key.len() + value.len(),
            BatchOperation::DeleteMetadata { key } => key.len(),
            BatchOperation::PutConsensus { key, value } => key.len() + value.len(),
        ...
        for operation in &operations {
            match operation {
                BatchOperation::PutAccount { address, account } => {
                    ...
                    // For now, serialize as simple bytes - replace with proper serialization later
                ...
                BatchOperation::DeleteContract { address } => {
                    // Note: Simplified deletion. In practice, delete all storage keys for the contract.
                    let cf_handle = self.engine.get_cf_handle("contracts")?;
                    let code_key = [b"code:", address.as_slice()].concat();
    ...
    fn operation_description(&self, operation: &BatchOperation) -> String {
        match operation {
            BatchOperation::PutAccount { address, .. } => {
                format!("PUT_ACCOUNT: {}", hex::encode(address.as_slice()))
            }
            BatchOperation::DeleteAccount { address } => {
                format!("DELETE_ACCOUNT: {}", hex::encode(address.as_slice()))
            }
            BatchOperation::PutTransaction { hash, data } => {
                format!(
                    "PUT_TRANSACTION: {} ({}) bytes",
                    hex::encode(hash),
                    data.len()
                )
            }
            ...
            BatchOperation::DeleteContract { address } => {
                format!("DELETE_CONTRACT: {}", hex::encode(address.as_slice()))
            }
            BatchOperation::PutDfsRef { hash, reference } => {
                format!(
                    "PUT_DFS_REF: {} ({} bytes)",
...
mod tests {
    ...
    async fn test_size_estimation() {
        ...
        batch
            .set_metadata("large_key", &vec![0u8; 1000])
            .await
            .unwrap();
