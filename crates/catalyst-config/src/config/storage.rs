    ...
    pub fn validate(&self) -> ConfigResult<()> {
        ...
        if self.dfs.enabled {
            if self.dfs.replication_factor == 0 {
                return Err(ConfigError::ValidationFailed(
                    "DFS replication factor must be greater than 0".to_string(),
                ));
            }

            if self.dfs.max_cache_size_bytes == 0 {
                return Err(ConfigError::ValidationFailed(
                    "DFS cache size must be greater than 0".to_string(),
                ));
            }
        }
        ...
        if self.cache.enabled {
            ...
            if total_layer_size > self.cache.max_cache_size_bytes {
                return Err(ConfigError::ValidationFailed(
                    "Sum of cache layer sizes exceeds max cache size".to_string(),
                ));
            }
        }
