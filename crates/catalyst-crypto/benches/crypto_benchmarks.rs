...
fn bench_signature_operations(c: &mut Criterion) {
    ...
    let signature = scheme.sign(&mut rng, keypair.private_key(), message).unwrap();
    group.bench_function("signature_verification", |b| {
        b.iter(|| {
            let _valid = scheme.verify(
                black_box(message),
                black_box(&signature),
...
fn bench_musig_operations(c: &mut Criterion) {
    ...
    for participant_count in [3, 5, 10, 20].iter() {
        ...
        for keypair in &keypairs {
            let sig = scheme.sign(&mut rng, keypair.private_key(), message).unwrap();
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    let _musig_sig = scheme.aggregate_signatures(
                        black_box(message),
                        black_box(&partial_signatures)
                    ).unwrap();
                });
            },
        );
        
        let musig_sig = scheme.aggregate_signatures(message, &partial_signatures).unwrap();
        group.bench_with_input(
            BenchmarkId::new("musig_verification", participant_count),
            participant_count,
            |b, _| {
                b.iter(|| {
                    let _valid = scheme.verify_aggregated(
                        black_box(message),
                        black_box(&musig_sig)
                    ).unwrap();
                });
...
fn bench_hash_operations(c: &mut Criterion) {
    ...
    for size in [32, 64, 256, 1024, 4096].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("blake2b_hash", size),
            size,
            |b, _| {
                b.iter(|| {
