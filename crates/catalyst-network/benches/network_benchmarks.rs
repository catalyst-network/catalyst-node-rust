...
fn benchmark_message_serialization(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000, 100000].iter() {
        group.bench_with_input(BenchmarkId::new("serialize", size), size, |b, &size| {
            ...
            b.iter(|| {
                let serialized = catalyst_utils::serialization::CatalystSerialize::serialize(
                    black_box(&message),
                )
                .unwrap();
        ...
        group.bench_with_input(BenchmarkId::new("deserialize", size), size, |b, &size| {
            ...
            let serialized =
                catalyst_utils::serialization::CatalystSerialize::serialize(&message).unwrap();

            b.iter(|| {
                let deserialized = catalyst_utils::serialization::CatalystDeserialize::deserialize(
                    black_box(&serialized),
                )
                .unwrap();
...
fn benchmark_reputation_updates(c: &mut Criterion) {
    ...
    c.bench_function("reputation_update", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            reputation
                .update_peer_score(black_box(peer_id), black_box(5.0), "benchmark test")
                .await;
        });
    });
}
...
use catalyst_network::{
    discovery::{DiscoveryMethod, PeerDiscovery},
    messaging::{MessageHandler, NetworkMessage, RoutingStrategy},
...
async fn test_network_service_lifecycle() {
    ...
    assert_eq!(
        ...
        catalyst_network::service::ServiceState::Stopped
    );
    ...
    assert_eq!(
        service.get_state().await,
        catalyst_network::service::ServiceState::Running
    );
...
async fn test_peer_discovery() {
    ...
    // Add peer
    let event = discovery
        .add_peer(peer_id, addresses.clone(), DiscoveryMethod::Manual)
        .await;

    // Verify event
    match event {
        catalyst_network::discovery::DiscoveryEvent::PeerDiscovered {
...
async fn test_reputation_management() {
    ...
    // Update score
    reputation
        .update_peer_score(peer_id, 10.0, "test increase")
        .await;
    ...
    reputation
        .update_peer_score(peer_id, -100.0, "test decrease")
        .await;
    assert!(!reputation.should_connect_to_peer(&peer_id).await);
}
...
async fn test_security_validation() {
    ...
    assert!(security
        .validate_peer_connection(&peer_id, &ip)
        .await
        .is_ok());
...
async fn test_gossip_protocol() {
    ...
    let message =
        NetworkMessage::new(&ping, "sender".to_string(), None, RoutingStrategy::Gossip).unwrap();
...
async fn test_message_serialization_roundtrip() {
    ...
    // Serialize
    let serialized =
        catalyst_utils::serialization::CatalystSerialize::serialize(&original).unwrap();

    // Deserialize
    let deserialized =
        catalyst_utils::serialization::CatalystDeserialize::deserialize(&serialized).unwrap();

    // Compare
    assert_eq!(original.routing.hop_count, deserialized.routing.hop_count);
    assert_eq!(
        original.transport.size_bytes,
        deserialized.transport.size_bytes
    );
...
impl TestNetwork {
    pub async fn new(node_count: usize) -> Self {
        ...
        for i in 0..node_count {
            ...
            config.peer.listen_addresses =
                vec![format!("/ip4/127.0.0.1/tcp/{}", 9000 + i).parse().unwrap()];
...
use catalyst_network::discovery::{DiscoveryMethod, PeerDiscovery};
use catalyst_network::messaging::{NetworkMessage, RoutingStrategy};
use catalyst_utils::network::PingMessage;
use libp2p::PeerId;
