...
impl LedgerStateUpdate {
    ...
    pub fn new(state_root: Hash, block_number: u64, timestamp: u64, producer: Address) -> Self {
        Self {
            state_root,
            block_number,
            timestamp,
    ...
    pub fn serialize(&self) -> Result<Vec<u8>, CatalystError> {
        serde_json::to_vec(self).map_err(|e| CatalystError::Serialization(e.to_string()))
    }

    /// Deserialize the update from bytes
    pub fn deserialize(data: &[u8]) -> Result<Self, CatalystError> {
        serde_json::from_slice(data).map_err(|e| CatalystError::Serialization(e.to_string()))
    }
}
