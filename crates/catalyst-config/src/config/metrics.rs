...
pub enum ExportTargetType {
    ...
    /// InfluxDB
    InfluxDb { url: String, database: String, token: Option<String> },
    /// StatsD
    StatsD { host: String, port: u16, prefix: Option<String> },
    /// HTTP endpoint
...
pub enum AlertCondition {
    ...
    RateOfChange { operator: ComparisonOperator, value: f64, duration_seconds: u64 },
...
impl MetricsConfig {
    ...
    pub fn mainnet() -> Self {
        ...
        config.storage.retention.category_policies = vec![
            CategoryRetentionPolicy {
        ...
        config.alerting.rules.extend(vec![
            ...
            AlertRule {
                ...
                metric: "up".to_string(),
                condition: AlertCondition::Absence { duration_seconds: 120 },
    ...
    pub fn validate(&self) -> ConfigResult<()> {
        ...
        if self.server.port == 0 {
            return Err(ConfigError::ValidationFailed("Metrics server port cannot be 0".to_string()));
        }
        
        if self.server.bind_address.is_empty() {
            return Err(ConfigError::ValidationFailed("Metrics server bind address cannot be empty".to_string()));
        }
        
        if self.server.endpoint_path.is_empty() {
            return Err(ConfigError::ValidationFailed("Metrics endpoint path cannot be empty".to_string()));
        }
        
        // Validate collection intervals
        if self.collection.collection_interval_seconds == 0 {
            return Err(ConfigError::ValidationFailed("Collection interval must be greater than 0".to_string()));
        }
        
        if self.collection.high_frequency_enabled && self.collection.high_frequency_interval_ms == 0 {
            return Err(ConfigError::ValidationFailed("High frequency interval must be greater than 0".to_string()));
        }
        
        // Validate storage configuration
        if let StorageBackend::Local = self.storage.backend {
            if self.storage.local.directory.as_os_str().is_empty() {
                return Err(ConfigError::ValidationFailed("Local storage directory cannot be empty".to_string()));
            }
            
            if self.storage.local.max_size_bytes == 0 {
                return Err(ConfigError::ValidationFailed("Local storage max size must be greater than 0".to_string()));
            }
        }
        
        // Validate retention policies
        if self.storage.retention.default_retention_seconds == 0 {
            return Err(ConfigError::ValidationFailed("Default retention period must be greater than 0".to_string()));
        }
        
        // Validate export configuration
        if self.export.enabled {
            if self.export.interval_seconds == 0 {
                return Err(ConfigError::ValidationFailed("Export interval must be greater than 0".to_string()));
            }
            
            if self.export.targets.is_empty() {
                return Err(ConfigError::ValidationFailed("At least one export target must be configured when export is enabled".to_string()));
            }
        }
        ...
        if self.alerting.enabled {
            for rule in &self.alerting.rules {
                if rule.name.is_empty() {
                    return Err(ConfigError::ValidationFailed("Alert rule name cannot be empty".to_string()));
                }
                
                if rule.metric.is_empty() {
                    return Err(ConfigError::ValidationFailed("Alert rule metric cannot be empty".to_string()));
                }
                
                if rule.evaluation_interval_seconds == 0 {
                    return Err(ConfigError::ValidationFailed("Alert rule evaluation interval must be greater than 0".to_string()));
                }
            }
            
            for channel in &self.alerting.channels {
                if channel.name.is_empty() {
                    return Err(ConfigError::ValidationFailed("Alert channel name cannot be empty".to_string()));
                }
            }
        }
        ...
        if self.performance.concurrency.worker_threads == 0 {
            return Err(ConfigError::ValidationFailed("Worker threads must be greater than 0".to_string()));
        }
        
        if self.performance.concurrency.queue_size == 0 {
            return Err(ConfigError::ValidationFailed("Queue size must be greater than 0".to_string()));
        }
        
        if self.performance.memory_management.max_memory_bytes == 0 {
            return Err(ConfigError::ValidationFailed("Max memory bytes must be greater than 0".to_string()));
        }
        ...
        if self.custom_metrics.enabled {
            if self.custom_metrics.dynamic_creation.enabled {
                if self.custom_metrics.dynamic_creation.max_dynamic_metrics == 0 {
                    return Err(ConfigError::ValidationFailed("Max dynamic metrics must be greater than 0".to_string()));
                }
                
                if self.custom_metrics.dynamic_creation.metric_lifetime_seconds == 0 {
                    return Err(ConfigError::ValidationFailed("Metric lifetime must be greater than 0".to_string()));
                }
            }
        }
        ...
        if let StorageBackend::TimeSeries(_) = self.storage.backend {
            if self.storage.tsdb.url.is_empty() {
                return Err(ConfigError::ValidationFailed("TSDB URL cannot be empty".to_string()));
            }
            
            if self.storage.tsdb.database.is_empty() {
                return Err(ConfigError::ValidationFailed("TSDB database name cannot be empty".to_string()));
            }
            
            if self.storage.tsdb.connection_timeout_ms == 0 {
                return Err(ConfigError::ValidationFailed("TSDB connection timeout must be greater than 0".to_string()));
            }
            
            if self.storage.tsdb.batch_size == 0 {
                return Err(ConfigError::ValidationFailed("TSDB batch size must be greater than 0".to_string()));
            }
        }
        ...
        if self.export.enabled {
            for target in &self.export.targets {
                if target.name.is_empty() {
                    return Err(ConfigError::ValidationFailed("Export target name cannot be empty".to_string()));
                }
                
                match &target.target_type {
                    ExportTargetType::Prometheus { gateway_url, job } => {
                        if gateway_url.is_empty() {
                            return Err(ConfigError::ValidationFailed("Prometheus gateway URL cannot be empty".to_string()));
                        }
                        if job.is_empty() {
                            return Err(ConfigError::ValidationFailed("Prometheus job name cannot be empty".to_string()));
                        }
                    }
                    ExportTargetType::InfluxDb { url, database, .. } => {
                        if url.is_empty() {
                            return Err(ConfigError::ValidationFailed("InfluxDB URL cannot be empty".to_string()));
                        }
                        if database.is_empty() {
                            return Err(ConfigError::ValidationFailed("InfluxDB database name cannot be empty".to_string()));
                        }
                    }
                    ExportTargetType::StatsD { host, port, .. } => {
                        if host.is_empty() {
                            return Err(ConfigError::ValidationFailed("StatsD host cannot be empty".to_string()));
                        }
                        if *port == 0 {
                            return Err(ConfigError::ValidationFailed("StatsD port cannot be 0".to_string()));
                        }
                    }
                    ExportTargetType::Http { url, .. } => {
                        if url.is_empty() {
                            return Err(ConfigError::ValidationFailed("HTTP export URL cannot be empty".to_string()));
                        }
                    }
                    ExportTargetType::File { path, .. } => {
                        if path.as_os_str().is_empty() {
                            return Err(ConfigError::ValidationFailed("File export path cannot be empty".to_string()));
                        }
                    }
                    ExportTargetType::Custom { config } => {
                        if config.is_empty() {
                            return Err(ConfigError::ValidationFailed("Custom export config cannot be empty".to_string()));
                        }
                    }
                }
            }
        ...
        if self.performance.aggregation.enabled {
            if self.performance.aggregation.window_seconds == 0 {
                return Err(ConfigError::ValidationFailed("Aggregation window must be greater than 0".to_string()));
            }
            
            if self.performance.aggregation.functions.is_empty() {
                return Err(ConfigError::ValidationFailed("At least one aggregation function must be specified".to_string()));
            }
        }
        ...
        if self.performance.memory_management.pressure_threshold < 0.0 || 
           self.performance.memory_management.pressure_threshold > 1.0 {
            return Err(ConfigError::ValidationFailed("Memory pressure threshold must be between 0.0 and 1.0".to_string()));
        }
        ...
        for rule in &self.alerting.rules {
            match &rule.condition {
                AlertCondition::Threshold { value, .. } => {
                    if value.is_nan() || value.is_infinite() {
                        return Err(ConfigError::ValidationFailed(
                            format!("Invalid threshold value for alert rule '{}'", rule.name)
                        ));
                    }
                }
                AlertCondition::RateOfChange { value, duration_seconds, .. } => {
                    if value.is_nan() || value.is_infinite() {
                        return Err(ConfigError::ValidationFailed(
                            format!("Invalid rate of change value for alert rule '{}'", rule.name)
                        ));
                    }
                    if *duration_seconds == 0 {
                        return Err(ConfigError::ValidationFailed(
                            format!("Rate of change duration must be greater than 0 for alert rule '{}'", rule.name)
                        ));
                    }
                }
                AlertCondition::Absence { duration_seconds } => {
                    if *duration_seconds == 0 {
                        return Err(ConfigError::ValidationFailed(
                            format!("Absence duration must be greater than 0 for alert rule '{}'", rule.name)
                        ));
                    }
                }
                AlertCondition::Custom { expression } => {
                    if expression.is_empty() {
                        return Err(ConfigError::ValidationFailed(
                            format!("Custom alert expression cannot be empty for rule '{}'", rule.name)
                        ));
                    }
                }
            }
    ...
    pub fn is_production_ready(&self) -> bool {
        ...
        if self.storage.retention.default_retention_seconds < 604800 { // Less than 1 week
            return false;
        }
