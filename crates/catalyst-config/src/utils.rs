...
use crate::loader::{FileLoader, EnvLoader};
use crate::config::metrics::StorageBackend;
use crate::config::logging::OutputType;
...
impl ConfigUtils {
    ...
    pub fn find_config_file(filename: &str) -> ConfigResult<PathBuf> {
        let search_paths = vec![
            ...
            dirs::config_dir().map(|d| d.join("catalyst")).unwrap_or_else(|| PathBuf::from(".catalyst")),
            ...
            std::env::current_exe()
                ...
                .map(|p| p.join("config")).unwrap_or_else(|| PathBuf::from("config")),
        ];
        ...
        Err(ConfigError::FileNotFound(format!(
            "Configuration file '{}' not found in standard locations", filename
        )))
    }
    ...
    pub fn validate_file_permissions<P: AsRef<Path>>(path: P) -> ConfigResult<()> {
        ...
        #[cfg(unix)]
        {
            ...
            if mode & 0o044 != 0 {
                return Err(ConfigError::ValidationFailed(
                    format!("Configuration file {} has overly permissive permissions", path.display())
                ));
            }
        }
        ...
        let template = match network {
            ...
            "custom" => {
                ...
                return Ok(toml::to_string_pretty(&config)
                    .map_err(|e| ConfigError::InvalidFormat(format!("Template generation failed: {}", e)))?);
            }
            _ => return Err(ConfigError::InvalidNetwork(format!("Unknown network template: {}", network))),
        };
    ...
    pub async fn load_with_fallbacks(primary_path: Option<PathBuf>) -> ConfigResult<crate::CatalystConfig> {
        // Try primary path first
        if let Some(path) = primary_path {
            if path.exists() {
                match crate::loader::FileLoader::load_auto(&path).await {
                    Ok(config) => return Ok(config),
                    Err(e) => eprintln!("Warning: Failed to load primary config {}: {}", path.display(), e),
                }
            }
        }
        ...
        match Self::find_config_file(&default_config_file) {
            Ok(path) => {
                match crate::loader::FileLoader::load_auto(&path).await {
                    Ok(config) => return Ok(config),
                    Err(e) => eprintln!("Warning: Failed to load default config {}: {}", path.display(), e),
                }
            }
...
impl ConfigWatcher {
    pub fn new<P: AsRef<Path>>(config_path: P, config: &crate::CatalystConfig) -> ConfigResult<Self> {
...
pub fn generate_config_template(network: &str) -> ConfigResult<String> {
    let template = match network {
        ...
        _ => return Err(ConfigError::InvalidFormat(
            format!("Unknown network template: {}", network)
        )),
    };
    Ok(template.to_string())
}