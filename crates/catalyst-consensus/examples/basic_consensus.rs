use catalyst_consensus::{
    CatalystConsensus, ConsensusConfig, Transaction,
    ConsensusProtocol, ConsensusMessage, ConsensusPhase  // Import the trait and types we need
};
...
async fn demonstrate_consensus_phases() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for (cycle, (node_id, consensus)) in nodes.iter_mut().enumerate() {
        ...
        println!("Initial state - Height: {}, Round: {}", state.current_height, state.current_round);
        ...
        println!("  Proposed block with {} transactions", proposal.transactions.len());
        ...
        println!("  Final state - Height: {}, Votes: {}", 
                final_state.current_height, final_state.votes.len());
