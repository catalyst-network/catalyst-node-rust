...
use catalyst_runtime_evm::{
    ...
    ContractDeployer, EvmConfig, EvmDebugger, EvmTransaction, FeeCalculator, GasMeter,
};
...
fn create_test_runtime() -> CatalystEvmRuntime<InMemoryDatabase> {
    ...
    let accounts = [
        (
            Address::from_str("0x742d35Cc6527C3c7Bb91B6B4d0e71b46C7E3e8F4").unwrap(),
            U256::from(100_000_000_000_000_000_000u64),
        ), // 100 ETH
        (
            Address::from_str("0x8ba1f109551bD432803012645Hac136c2Bb25F00c").unwrap(),
            U256::from(50_000_000_000_000_000_000u64),
        ), // 50 ETH
        (
            Address::from_str("0x1234567890123456789012345678901234567890").unwrap(),
            U256::from(25_000_000_000_000_000_000u64),
        ), // 25 ETH
    ];
...
async fn test_simple_eth_transfer() {
    ...
    assert_eq!(
        result.gas_used(),
        21000,
        "Should use exactly 21000 gas for simple transfer"
    );
    assert!(
        result.return_data().is_empty(),
        "Simple transfer should have no return data"
    );
    assert!(
        result.logs().is_empty(),
        "Simple transfer should emit no logs"
    );
}
...
async fn test_contract_deployment() {
    ...
    let result = runtime
        .execute_transaction(&deploy_tx, &block_env)
        .await
        .unwrap();
    ...
    assert!(
        ...
        "Contract deployment should use more than 21000 gas"
    );
    assert!(
        result.base_result.created_address.is_some(),
...
async fn test_contract_interaction() {
    ...
    let call_result = runtime
        .execute_transaction(&call_tx, &block_env)
        .await
        .unwrap();

    assert!(call_result.success(), "Contract call should succeed");
    assert!(
        call_result.gas_used() > 21000,
        "Contract call should use more than transfer gas"
    );
}
...
async fn test_contract_deployer_create_addresses() {
    ...
    let create2_address1 = deployer
        .deploy_contract(deployer_address, bytecode.clone(), vec![], Some(salt1))
        .unwrap();

    let create2_address2 = deployer
        .deploy_contract(deployer_address, bytecode.clone(), vec![], Some(salt2))
        .unwrap();
    ...
    let create2_address1_repeat = deployer
        .deploy_contract(deployer_address, bytecode, vec![], Some(salt1))
        .unwrap();
