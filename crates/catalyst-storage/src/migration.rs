...
impl MigrationManager {
    ...
    fn register_builtin_migrations(&mut self) {
            ...
            Box::new(|engine| {
                Box::pin(async move {
                    ...
                    engine.put(
                        "metadata",
                        b"created_at",
                        &catalyst_utils::utils::current_timestamp().to_le_bytes(),
                    )?;
        ...
        .with_can_apply(Box::new(|engine| {
            Box::pin(async move {
                ...
                let has_all_cfs = required_cfs
                    .iter()
                    .all(|cf| existing_cfs.contains(&cf.to_string()));
    ...
    async fn add_to_history(&self, record: MigrationRecord) -> StorageResult<()> {
        ...
        let history_key = b"migration_history";
        let history_data =
            serde_json::to_vec(&history).map_err(|e| StorageError::serialization(e.to_string()))?;
