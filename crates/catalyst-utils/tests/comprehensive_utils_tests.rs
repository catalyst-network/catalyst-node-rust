...
use catalyst_utils::{
    CatalystResult, CatalystError,
    Hash, Address, SystemInfo,
    TransactionEntry, TransactionStatus,
    LedgerStateUpdate, AccountUpdate,
    utils::{RateLimiter, current_timestamp, format_bytes, bytes_to_hex, hex_to_bytes},
    crypto::hash_data,
    logging::{LogConfig, CatalystLogger, LogCategory, LogLevel},
    consensus_error,
};
...
fn test_logging_system() {
    ...
    logger.warn(LogCategory::Network, "Warning message").unwrap();
    logger.error(LogCategory::Consensus, "Error message").unwrap();
    
    // Test with fields
    let fields = vec![
        ("key1", catalyst_utils::logging::LogValue::String("value1".to_string())),
