...
pub mod utils {
    ...
    pub fn deserialize_bytes<R: Read>(reader: &mut R) -> CatalystResult<Vec<u8>> {
        ...
        if len > 100_000_000 { // 100MB limit
            return Err(CatalystError::Serialization(
                format!("Data length {} exceeds maximum allowed size", len)
            ));
        }
    ...
    pub fn serialize_vec<W: Write, T: CatalystSerialize>(
        ...
        writer.write_all(&len.to_le_bytes())
            .map_err(|e| CatalystError::Serialization(format!("Failed to write vec length: {}", e)))?;
    ...
    pub fn deserialize_vec<R: Read, T: CatalystDeserialize>(
        ...
        let mut len_bytes = [0u8; 4];
        reader.read_exact(&mut len_bytes)
            .map_err(|e| CatalystError::Serialization(format!("Failed to read vec length: {}", e)))?;
        
        let len = u32::from_le_bytes(len_bytes) as usize;
        
        // Sanity check for vector length
        if len > 1_000_000 { // 1M items max
            return Err(CatalystError::Serialization(
                format!("Vector length {} exceeds maximum allowed size", len)
            ));
        }
...
impl CatalystDeserialize for u8 {
    fn deserialize(data: &[u8]) -> CatalystResult<Self> {
        if data.len() < 1 {
            return Err(CatalystError::Serialization("Insufficient data for u8".to_string()));
        }
        Ok(data[0])
    }
...
impl CatalystDeserialize for u32 {
    fn deserialize(data: &[u8]) -> CatalystResult<Self> {
        if data.len() < 4 {
            return Err(CatalystError::Serialization("Insufficient data for u32".to_string()));
        }
        let mut bytes = [0u8; 4];
...
impl CatalystDeserialize for u64 {
    fn deserialize(data: &[u8]) -> CatalystResult<Self> {
        if data.len() < 8 {
            return Err(CatalystError::Serialization("Insufficient data for u64".to_string()));
        }
        let mut bytes = [0u8; 8];
