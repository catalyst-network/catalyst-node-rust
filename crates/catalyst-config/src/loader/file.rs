...
impl FileLoader {
    ...
    pub async fn load_toml<P: AsRef<Path>>(path: P) -> ConfigResult<CatalystConfig> {
        ...
        let config: CatalystConfig = toml::from_str(&content).map_err(ConfigError::Toml)?;
        config.validate()?;
        Ok(config)
    }
    ...
    pub async fn load_json<P: AsRef<Path>>(path: P) -> ConfigResult<CatalystConfig> {
        let path = path.as_ref();
        let content = fs::read_to_string(path).await.map_err(ConfigError::Io)?;
        let config: CatalystConfig = serde_json::from_str(&content).map_err(ConfigError::Json)?;
        config.validate()?;
        Ok(config)
    }
