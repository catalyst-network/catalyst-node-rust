...
| **8 — state-root certification** | Make `state_root` a BFT-certified fact, not a single peer's claim (closes the gap in [`consensus-reliability-review-2026-07.md`](./consensus-reliability-review-2026-07.md)) | **Done (2026-07-12):** [ADR 0002](./adr/0002-canonical-state-root-and-proofs.md) `LsuStateRootCertificateV1` (`catalyst-consensus/src/state_root_finality.rs`); `StateRootAttestationMsg`/`StateRootFinalityCidMsg` gossip + aggregation (`node.rs`); apply-path gate `verify_state_root_finality_for_apply` behind `[consensus] require_state_root_finality` (default `false`, `CATALYST_REQUIRE_STATE_ROOT_FINALITY` override); fork-choice tie-break `ForkChoiceCandidate.state_root_certified`. **Still TBD:** flip default to `true` for mainnet once every peer gossips state-root CIDs reliably (mirrors Phase 7's ADR 0001 rollout). | Phase 0 |
| **9 — reconcile self-heal (near-head)** | Always have a checkpoint anchor available shortly after restart/reset | **Done (2026-07-12):** every process writes a checkpoint on its first successful apply, not only at the periodic `CATALYST_CHECKPOINT_EVERY_CYCLES` boundary; new `consensus_fork_reconcile_no_checkpoint_total` metric distinguishes the remaining truly-stuck case; operator runbook entry in `node-operator-guide.md`. | Phase 0 |
| **10 — multi-process restart-recovery CI gate** | CI coverage for "kill a validator, resume it on stale/rolled-back data" — the trigger pattern behind the 2026-07-03 fork | `scripts/consensus-restart-recovery-e2e.sh` (real 3-process testnet, `db-backup`/`db-restore` to simulate stale resume, asserts convergence); `make consensus-restart-recovery-e2e`; nightly CI job (`RUN_CONSENSUS_E2E=1`, not on every PR — matches §8 exit criteria). | Phase 0 |

**Parallel tracks:** Phase **5** and **6** can run beside **1**; **4** needs design before coding; **7** is last; **8**–**10** are independent of **1**–**7** and of each other.
...
| 3.5 | **`state_root` itself is certified, not just the recipe:** ADR 0001 certifies `lsu_hash` (the recipe); it says nothing about whether the *result* of applying it is agreed. Implement [ADR 0002](./adr/0002-canonical-state-root-and-proofs.md) `LsuStateRootCertificateV1`; gate the trusted/catch-up apply path on it via `[consensus] require_state_root_finality` + env override. | `crates/catalyst-consensus/src/state_root_finality.rs`, `verify_state_root_finality_for_apply` in `node.rs` |
...
| 2026-07-12 | **Progress (batch 16, Phases 8–9):** Triggered by [`consensus-reliability-review-2026-07.md`](./consensus-reliability-review-2026-07.md) (2026-07-03 US/Asia fork: agreed `lsu_hash`, diverged `state_root`). **§3.5 / Phase 8:** [ADR 0002](./adr/0002-canonical-state-root-and-proofs.md) — `LsuStateRootCertificateV1` (`catalyst-consensus/src/state_root_finality.rs`, `H_root` domain-separated from ADR 0001's `H_cert`); attestation gossip + aggregation mirroring ADR 0001's flow (`StateRootAttestationMsg`/`StateRootFinalityCidMsg`, `ingest_state_root_attestation`, `try_publish_state_root_certificate_from_bucket` in `node.rs`); apply-path gate `verify_state_root_finality_for_apply` behind `[consensus] require_state_root_finality` (default `false`) + `CATALYST_REQUIRE_STATE_ROOT_FINALITY`; fork-choice tie-break `ForkChoiceCandidate.state_root_certified` (`fork_choice.rs`). **Phase 9:** every process now writes a checkpoint on its first successful apply (not only at the periodic interval), closing the near-head no-checkpoint reconcile gap; new `consensus_fork_reconcile_no_checkpoint_total` metric; operator runbook entry. Docs: `consensus-quorum-and-fork-choice.md` state-root tie-break section, `protocol-params.md` new TOML/env rows. **Next:** flip `require_state_root_finality` default to `true` for mainnet once attestation gossip is fleet-wide (Phase 8); Phase 10 multi-process restart-recovery CI gate; separately-flagged follow-up in ADR 0002 to fix `merkle::verify_proof`'s self-declared (not key-derived) proof direction. |
