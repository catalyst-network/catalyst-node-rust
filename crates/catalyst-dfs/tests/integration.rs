...
use catalyst_dfs::{
    CategorizedStorage, ContentCategory, ContentProvider, ContentReplicator, DfsConfig, DfsFactory,
    DfsService, DhtContentProvider, DistributedFileSystem, IpfsFactory, ReplicationLevel,
};
use std::sync::Arc;
...
async fn test_categorized_storage_integration() {
    ...
    let contract_cid = dfs
        .put_categorized(contract_data.clone(), ContentCategory::Contract)
        .await
        .unwrap();
    let media_cid = dfs
        .put_categorized(media_data.clone(), ContentCategory::Media)
        .await
        .unwrap();
    let app_cid = dfs
        .put_categorized(app_data.clone(), ContentCategory::AppData)
        .await
        .unwrap();

    // Verify content by category
    let contracts = dfs
        .list_by_category(ContentCategory::Contract)
        .await
        .unwrap();
    ...
    let app_files = dfs
        .list_by_category(ContentCategory::AppData)
        .await
        .unwrap();
...
async fn test_error_handling_integration() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        catalyst_dfs::DfsError::NotFound(_)
    ));
...
async fn test_storage_limits() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        catalyst_dfs::DfsError::Storage(_)
    ));
}
