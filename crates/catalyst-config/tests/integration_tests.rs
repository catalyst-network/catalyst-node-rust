...
async fn test_network_config_integration() {
    ...
    for network in networks {
        ...
        let validator = ConfigValidator::new();
        let validation_result = validator.validate(&config);
        assert!(validation_result.is_ok(), "Network {:?} config validation failed", network);
    }
}
...
"#;
    ...
    // Start hot reload
    let mut hot_reload = HotReloadManager::new(config.clone(), config_path.clone(), Network::Devnet);
    let mut event_receiver = hot_reload.start_auto_reload().await.unwrap();
    
    // Verify environment overrides were applied
    assert_eq!(hot_reload.get_current_config().node.id, "env_full_feature_node");
    assert_eq!(hot_reload.get_current_config().network.max_peers, 300);
    
    // Test manual config update
    hot_reload.update_config_value("consensus.cycle_time_ms", "50000").await.unwrap();
    assert_eq!(hot_reload.get_current_config().consensus.cycle_time_ms, 50000);
